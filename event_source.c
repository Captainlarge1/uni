...
static pthread_mutex_t event_running_mutex = PTHREAD_MUTEX_INITIALIZER; // Add mutex for event_running
...
void event_source_start(useconds_t interval) {
    if (event_running) return; // Already running
    pthread_mutex_lock(&event_running_mutex);
    event_running = 1;
    pthread_mutex_unlock(&event_running_mutex);
